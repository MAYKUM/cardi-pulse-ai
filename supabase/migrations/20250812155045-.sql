-- 1) Add 'ophthalmology' to medical_specialty enum safely
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    WHERE t.typname = 'medical_specialty' AND e.enumlabel = 'ophthalmology'
  ) THEN
    ALTER TYPE public.medical_specialty ADD VALUE 'ophthalmology';
  END IF;
END $$;

-- 2) Update seed_patients_for_current_user to handle ophthalmology
